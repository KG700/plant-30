...
describe("Day", () => {
  ...
  it("calls delete endpoint when plant delete button pressed", async () => {
    ...
    await waitFor(() => {
      const onionItem: HTMLLIElement =
        screen.getByText("onion").closest("li") || new HTMLLIElement();
  ...
  it("renders error message if plant fails to delete", async () => {
    ...
    waitFor(() => {
      expect(
        screen.getByText("Failed to delete plant from list"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("shows no plants added message if no plants are fetched", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText("You have not added any plants for today yet"),
      ).toBeInTheDocument();
    });
  });
  ...
  it("throws error if plants fail to fetch", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Error fetching the plants you have eaten today"),
      ).toBeInTheDocument();
    });
  });
});
