...
describe("CreateNewPlant", () => {
  ...
  it("renders the create new plant input and dropdown", () => {
    ...
    const inputField: HTMLInputElement =
      screen.getByLabelText("enter-new-plant");
    const selectField: HTMLSelectElement =
  ...
  it("updates selected category on select change", () => {
    ...
    fireEvent.change(selectField, {
      target: { value: PlantCategories.vegetable },
    });
  ...
  it("displays error message when plant fails to be created.", async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByText(
          "Failed to create the new plant, beetroot. Please try again",
        ),
      ).toBeInTheDocument();
    });
